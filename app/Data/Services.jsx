import {
  FaBriefcase,
  FaBullhorn,
  FaCloud,
  FaLaptop,
  FaMale,
  FaMixcloud,
  FaRobot,
  FaRocket,
  FaUser,
  FaUsers,
} from "react-icons/fa";
import { FaGears, FaMobileScreenButton } from "react-icons/fa6";
import { IoCartOutline } from "react-icons/io5";

export const services = [
  {
    title: "Web Design & Development",
    description:
      "We create a smooth user experience for every web design and development project we take.",
    icon: FaLaptop,
    image: "/images/ser1.jpg",
    link: "/web-design-development",
  },
  {
    title: "Mobile App Development",
    description:
      "Codesinc enjoins a hallmark of projects in mobile application development and contributed to the industry.",
    icon: FaMobileScreenButton,
    image: "/images/ser2.jpg",
    link: "/mobile-application-development",
  },
  {
    title: "Software Development",
    description:
      "For various startups and established businesses alike, our exceptional software development company.",
    icon: FaGears,
    image: "/images/ser3.jpg",
    link: "/software-development",
  },
  {
    title: "Startup Solution",
    description:
      "We are the enablers of tech-savvy and budding entrepreneurs. We groom and nurture entrepreneurs from various industries.",
    icon: FaRocket,
    image: "/images/ser4.jpg",
    link: "/startup",
  },
  {
    title: "DevOps",
    description:
      "Codesinc would like to offer its services to move your workload to Azure with our Free migration and DevOps services during the lockdown!",
    icon: FaMixcloud,
    image: "/images/ser5.jpg",
    link: "/cloudservices",
  },
  {
    title: "Artificial Intelligence",
    description:
      "From dedicated and multi-tiered teams of professionals to simple staff augmentation, we offer exceptional service of resource allocation.",
    icon: FaRobot,
    image: "/images/ser6.jpg",
    link: "/web-design-development",
  },
  {
    title: "Ecommerce Solution",
    description:
      "E-commerce solution requires a technically holistic approach and it caters to all aspects including, design, development and e-cart.",
    icon: IoCartOutline,
    image: "/images/ser7.jpg",
    link: "/ecommerce-development",
  },
  {
    title: "Managed Cloud Hosting",
    description:
      "Codesinc Cloud Management Services drives us to create a customized solution that addresses all unique requirements.",
    icon: FaCloud,
    image: "/images/ser8.jpg",
    link: "/managed-cloud-hosting",
  },
  {
    title: "IT Resource Allocation",
    description:
      "Codesinc IT allocation service is all about managing human resources frontiers including tracking, optimization, and stages of analysis.",
    icon: FaUsers,
    image: "/images/ser9.jpg",
    link: "/it-resource-management",
  },
  {
    title: "Business Intelligence",
    description:
      "One of our hallmarks is Business Intelligence and Analytics services. It offers a unique basis for insights.",
    icon: FaBriefcase,
    image: "/images/ser10.jpg",
    link: "/business-intelligence",
  },
  {
    title: "Business Process Outsourcing",
    description:
      "We have bespoke cost-effective, and quite flexible Business Process Outsourcing services for clients to enable them across their journey.",
    icon: FaMale,
    image: "/images/ser11.jpg",
    link: "/business-process-outsourcing",
  },
  // {
  //   title: "Digital Marketing",
  //   description:
  //     "To establish the marketing possibilities of digital services, you have to crack the supremacy of social media platforms.",
  //   icon: FaBullhorn,
  //   image: "/images/ser12.jpg",
  //   link: "/web-design-development",
  // },
];

export const ServicesItems = [
  {
    icon: "/images/chose-1.png",
    title: "ECOMMERCE",
    description: "Website Development",
    items: [
      {
        name: "drupal",
        image: "/images/portfolio/web-01.png",
        link: "/drupal-development",
      },
      {
        name: "woocommerce",
        image: "/images/portfolio/web-02.png",
        link: "/woocommerce-development",
      },
      // {
      //   name: "Magento",
      //   image: "/images/portfolio/web-03.png",
      //   link: "/magento",
      // },
      {
        name: "opencart",
        image: "/images/portfolio/web-04.png",
        link: "/opencart-development",
      },
      {
        name: "prestashop",
        image: "/images/portfolio/web-05.png",
        link: "prestashop-development",
      },
      {
        name: "shopify",
        image: "/images/portfolio/web-06.png",
        link: "/shopify-development",
      },
    ],
  },
  {
    icon: "/images/chose-2.png",
    title: "CMS/CUSTOM",
    description: "Website Development",
    items: [
      {
        name: "WordPress",
        image: "/images/portfolio/web-07.png",
        link: "/wordpress-development",
      },
      {
        name: "cakephp",
        image: "/images/portfolio/web-08.png",
        link: "/cakephp-development",
      },
      {
        name: "asp-net-development",
        image: "/images/portfolio/web-09.png",
        link: "/asp-net-development",
      },
      {
        name: "zend-development",
        image: "/images/portfolio/web-10.png",
        link: "/zend-development",
      },
      {
        name: "php-development",
        image: "/images/portfolio/web-11.png",
        link: "/php-development",
      },
      {
        name: "angular-js-development",
        image: "/images/portfolio/web-13.png",
        link: "/angular-js-development",
      },
    ],
  },
];
export const application = [
  {
    icon: "/images/chose-3.png",
    title: "APPLICATION",
    description: "Website Development",
    items: [
      {
        name: "E-Learning",
        image: "/images/portfolio/23.png",
        link: "/elearning",
      },
      {
        name: "Social Networking",
        image: "/images/portfolio/24.png",
        link: "/social-networking",
      },
      {
        name: "CRM",
        image: "/images/portfolio/25.png",
        link: "/crm",
      },
      {
        name: "CRM",
        image: "/images/portfolio/26.png",
        link: "/crm",
      },
      {
        name: "CRM",
        image: "/images/portfolio/27.png",
        link: "/crm",
      },
      {
        name: "CRM",
        image: "/images/portfolio/28.png",
        link: "/crm",
      },
    ],
  },
];
